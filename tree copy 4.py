...
def main(boards: list):
    for line in lines:
        for url in boards:  # tqdm(boards):
            ...
            def append_tree(tree, hand_before, hand, subtree):
                keys = [i[0] for i in hand_before if i[1] == 1]
                if len(keys) == 0:
                    tree[hand] = subtree
                else:
                    reduce(lambda x, y: x[y], [tree] + keys)[hand] = subtree

            def step(df: pd.DataFrame, hand_before, tree=tree, temp_step=False):
                a = df.copy()
                ...
                not_approved = (
                    ...
                    or min([bc_1_weight, bc_0_weight]) < 0.005
                    or max([bc_1_result_sort[0][1], bc_0_result_sort[0][1]]) < 66
                )
                if not_approved:
                    temp_tree = {}
                    step(a[a[best_cut[0]]], [], temp_tree, True)
                    ...
                    append_tree(tree, hand_before, best_cut[0], {})
                    if temp_step:
                        return
                    if not temp_step:
                        if best_cut[2] > 0:
                        ...
                        step(a[a[best_cut[0]]], hand_before + [(best_cut[0], 1)])

                        step(a[~a[best_cut[0]]], hand_before + [(best_cut[0], 0)])
