...
class Cards:
    def __init__(self, a: pd.DataFrame) -> None:
        self.cards = np.array(
            [
                [[card_values[c[i]], suit_values[c[i + 1]]] for i in range(0, 8, 2)]
                for c in a.index
            ]
        )
        ...
        if self.cards.shape[0] > 10:
            return "\n".join(
                [
                    ...
                    for cards in np.vstack([self.cards[:rows], self.cards[-rows:]])
                ]
            )
        return "\n".join(
            [
...
class Board:
    ...
    def str8(self):
        def str8_wheel(ranks: np.array):
            three_groups = [
                i for i in combinations(sorted(ranks), 3) if np.max(i) - np.min(i) <= 4
            ]
            ...
            for i in three_groups:
                possible = [
                    j
                    for j in range(max(np.min(i) - 2, 1), min(np.max(i) + 3, 15))
                    if j not in i
                ]

                for n, k in enumerate(possible[:-1]):
                    new5 = np.concatenate((i, np.array([k, possible[n + 1]])))

                    if np.max(new5) - np.min(new5) == 4:
                        to_append = [k, possible[n + 1]]
                        to_append = sorted([m if m != 1 else 14 for m in to_append])

                        str8_cards.append(tuple(to_append + [np.max(new5)]))
        ...
        for n in [2, 3, 4]:
            for cardc in combinations(str8_cards, n):
                str8s = [
                    ...
                    for i in str_draws
                    if any(
                        [j[0] == i[0] and j[1] == i[1] for j in combinations(cardc, 2)]
                    )
                ]
    ...
    def sdbl(self):
        ...
        sd = np.array(
            [
                ...
                if self.str8_draw[i][0] == 2 and self.str8_draw[i][1] and len(i) == 2
            ]
        )
    ...
    @property
    def is_sdbl(self):
        return len(self.sdbl) > 0

    @property
    def is_sd(self):
        return len(self.str8_draw) > 0
    ...
    @property
    def is_doublepaired(self):
        return len(self.uranks[1][self.uranks[1] > 1]) > 1 or np.max(self.uranks[1]) > 2
