...
class Board():
    def __init__(self, boardStringList):
        self.board = np.sort(np.array([card_values[i] for i in boardStringList]))
        self.boardStringList=boardStringList

    @property
    def bsl(self): #get numpy array
    ...
    @property
    def paired(self):
        _, unique_board_counts = np.unique(ranks(self.board), return_counts=True)
    ...
    def __repr__(self):
        return np.array2string(self.board, formatter={'int': lambda x: card_names[x]})