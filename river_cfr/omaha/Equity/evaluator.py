...
class Evaluator(object):
    ...
    def __init__(self):
        ...
        self.hand_size_map = {5: self._five, 6: self._six, 9: self._seven}
    ...
    def _five(self, cards):
        ...
        if cards[0] & cards[1] & cards[2] & cards[3] & cards[4] & 0xF000:
            ...
            try:
                result = self.table.flush_lookup[prime]
            except:
                prime = Card.prime_product_from_hand(cards)
                result = self.table.unsuited_lookup[prime]
                return result
            return result
        # otherwise
        else:
    ...
    def _six(self, cards):
        ...
        allboard = itertools.combinations(cards[0:4], 3)

        allhand = list(itertools.combinations(cards[5:9], 2))

        for board in allboard:
            for hand in allhand:
                score = self._five(board + hand)
                if score < minimum:
                    minimum = score
    ...
    def _seven(self, cards):
        ...
        """
        minimum = LookupTable.MAX_HIGH_CARD
        # 3at választunk boardrol (elso5), 2ot handbol (utolso 4)

        allboard = itertools.combinations(cards[0:5], 3)
    ...
    def get_rank_class(self, hr):
        ...
        if hr >= 0 and hr <= LookupTable.MAX_STRAIGHT_FLUSH:
            return LookupTable.MAX_TO_RANK_CLASS[
