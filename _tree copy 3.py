...
def main(result_list, boards: list):
    ...
    for url in tqdm(boards):
        ...
        for line in a:
            ...
            def step(df: pd.DataFrame, hand_before):
                ...
                #     return
                approved = not (
                    bc_1_result_sort[0][0]
                    == bc_0_result_sort[0][0]
    ...
    # result_list = sorted(
    #     result_list, key=lambda x: card_values[x["board"][4]], reverse=True
    # )
    # result_list = sorted(
    #     result_list, key=lambda x: card_values[x["board"][2]], reverse=True
    # )
    # result_list = sorted(
    #     result_list, key=lambda x: card_values[x["board"][0]], reverse=True
    # )

    # filename = f"/Users/barrybaker/Documents/blackcard2/full_display/src/assets/trees/50_BTN_BB_SRP_False_False_True_False_flop_BTN.json"
    # with open(
    #     filename,
    #     "w",
    # ) as fp:
        ...
        if len(chunks) <= 9:
            ...
            for type in [
                ...
                for hero in poslist:
                    ...
                    for i in result:
                        if i["hero"] == hero and (
                            (type[3] and (len(Board(i["board"]).flush) > 0) == type[3])
                            or (
                                not type[3]
                                and Board(i["board"]).is_paired == type[0]
                                and (len(Board(i["board"]).str8) > 0) == type[1]
                                and (len(Board(i["board"]).fd) > 0) == type[2]
