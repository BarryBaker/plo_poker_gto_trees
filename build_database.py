...
def main(result_list, boards: list):
    for url in boards:  # tqdm(boards):
        ...
        for line in a:
            ...
            strat.drop(["combo"], axis=1, inplace=True)
            strat = strat.astype("uint8")
            print(url, board, line, strat, actions)
            strat.to_csv(f"accesories/dfs/{board.string_cards}_{line}.csv")
            tree = {}
            if len(actions) > 1:
